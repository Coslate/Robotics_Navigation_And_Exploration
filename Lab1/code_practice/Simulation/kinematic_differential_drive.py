...
class KinematicModelDifferentialDrive(KinematicModel):
    ...
    def step(self, state:State, cstate:ControlState) -> State:
        ...
        x1dot = self.r*np.deg2rad(cstate.rw)/2.0
        w1    = np.rad2deg(self.r*np.deg2rad(cstate.rw)/(2.0 * self.l))
        x2dot = self.r*np.deg2rad(cstate.lw)/2.0
        w2    = -np.rad2deg(self.r*np.deg2rad(cstate.lw)/(2.0 * self.l))
        v     = x1dot + x2dot
        w     = w1 + w2
